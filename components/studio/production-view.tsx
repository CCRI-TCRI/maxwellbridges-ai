...
export function ProductionView() {
      ...
      project.scenes.flatMap((scene) =>
        project.panels
          ...
          .map((p) => ({
            ...
            shot: p.shotNumber,
            type: shotTypeLabel(p.shotType),
            angle: p.angle,
            movement: movementLabel(p.movement),
            lens: p.lens,
            duration: p.duration,
            cast: p.stage.filter((s) => !s.isProp).map((s) => s.label).join(" / "),
  ...
  const cast = useMemo(() => {
    ...
    for (const p of project.panels) for (const s of p.stage) if (!s.isProp) set.add(s.label);
    return Array.from(set);
  }, [project]);

  const props = useMemo(() => {
    const set = new Set<string>();
    for (const sc of project.scenes) for (const pr of sc.propList) set.add(pr);
    for (const p of project.panels) for (const s of p.stage) if (s.isProp) set.add(s.label);
    return Array.from(set);
  }, [project]);
  ...
  const equipment = useMemo(() => {
    ...
    for (const p of project.panels) {
      ...
      if (p.movement === "dolly-in" || p.movement === "dolly-out" || p.movement === "tracking")
        movements.add("Dolly + track");
  ...
  // Shooting order: group by location to minimize company moves.
  const shootingOrder = useMemo(() => {
    const byLoc = new Map<string, number[]>();
    for (const sc of project.scenes) {
      const arr = byLoc.get(sc.location) ?? [];
      arr.push(sc.number);
      byLoc.set(sc.location, arr);
    }
  ...
  const exportShotList = () => {
    ...
    const csv = [
      header.join(","),
      ...rows.map((r) =>
        [r.scene, r.heading, r.shot, r.type, r.angle, r.movement, r.lens, r.duration, r.cast, r.action]
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="flex items-center gap-2 font-semibold text-lg">
            <ListChecks className="size-5" /> Production Mode
          </h2>
          <p className="text-muted-foreground text-sm">
            {rows.length} shots · {project.scenes.length} scenes · est. {Math.round(totalRuntime)}s runtime
          </p>
        </div>
        <Button onClick={exportShotList} variant="outline">
          <Download className="size-4" /> Export Shot List (CSV)
        </Button>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-left text-xs">
          <thead className="bg-secondary/60 text-muted-foreground uppercase">
            <tr>
              {["Sc", "Shot", "Type", "Angle", "Move", "Lens", "Dur", "Cast", "Action"].map((h) => (
                <th className="whitespace-nowrap px-2.5 py-2 font-medium" key={h}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
        ...
        <div className="rounded-lg border border-border p-3">
          ...
          <ol className="flex flex-col gap-1.5 text-xs">
            {shootingOrder.map(([loc, scenes], i) => (
              <li className="flex gap-2" key={loc}>
                <span className="font-mono text-muted-foreground">{i + 1}.</span>
                <span>
                  <span className="font-medium">{loc}</span>
                  <span className="text-muted-foreground"> — scenes {scenes.join(", ")}</span>
                </span>
              </li>
            ))}
          </ol>
    ...
    <div className="rounded-lg border border-border p-3">
      ...
      {items.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1 text-xs">
          {items.map((it) => (
            <li className="flex items-center gap-2" key={it}>
              <span className="size-3 shrink-0 rounded-sm border border-border" />
              {it}
            </li>
          ))}
        </ul>
