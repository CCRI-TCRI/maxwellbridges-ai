    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 font-semibold text-sm">
          <User className="size-4" /> Characters
        </h2>
        ...
        {characters.map((c) => (
          <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3" key={c.id}>
            ...
            <div className="flex flex-wrap gap-1">
              {CHARACTER_COLORS.map((col) => (
                <button
                  ...
                  style={{ background: col, outline: c.color === col ? "2px solid var(--ring)" : "none" }}
                  type="button"
                />
              ))}
            </div>
            <div className="grid grid-cols-2 gap-2">
              <Field label="Height">
                <TextInput onChange={(e) => update(c.id, { height: e.target.value })} value={c.height} />
              </Field>
              <Field label="Hairstyle">
                <TextInput onChange={(e) => update(c.id, { hairstyle: e.target.value })} value={c.hairstyle} />
              </Field>
            </div>
            <Field label="Clothing">
              <TextInput onChange={(e) => update(c.id, { clothing: e.target.value })} value={c.clothing} />
            </Field>
            <Field label="Accessories">
              <TextInput onChange={(e) => update(c.id, { accessories: e.target.value })} value={c.accessories} />
            </Field>
            <Field label="Notes">
              <TextArea onChange={(e) => update(c.id, { notes: e.target.value })} value={c.notes} />
            </Field>
          </div>
        ))}
      </div>
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3" key={l.id}>
            <div className="flex items-center gap-2">
              <TextInput
                ...
                value={l.name}
              />
              <button className="rounded p-1 text-muted-foreground hover:text-destructive" onClick={() => del(l.id)} type="button">
                <Trash2 className="size-4" />
              </button>
            </div>
            <div className="flex h-6 overflow-hidden rounded">
              {l.palette.map((hex, i) => (
                <div className="flex-1" key={i} style={{ background: hex }} title={hex} />
              ))}
            </div>
            <Field label="Type">
              <TextInput onChange={(e) => update(l.id, { type: e.target.value })} value={l.type} />
            </Field>
            <Field label="Ambient light">
              <TextInput onChange={(e) => update(l.id, { ambientLight: e.target.value })} value={l.ambientLight} />
            </Field>
            <Field label="Palette (comma-separated hex)">
              <TextInput
                onChange={(e) =>
                  update(l.id, { palette: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) })
                }
                value={l.palette.join(", ")}
              />
            </Field>
            <Field label="Description">
              <TextArea onChange={(e) => update(l.id, { description: e.target.value })} value={l.description} />
            </Field>
          </div>
        ))}
      </div>
