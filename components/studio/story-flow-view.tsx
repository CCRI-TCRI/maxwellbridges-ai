...
function PanelNode({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div
      className="w-[120px] rounded-md border bg-card px-2 py-1.5 text-left shadow-sm"
...
export function StoryFlowView() {
  ...
  if (project.scenes.length === 0) {
    return <EmptyState title="No story to flow yet" hint="Add scenes and shots to visualize the story's shape." />;
  }
  ...
  return (
    <div className="h-full w-full">
      <ReactFlow
        ...
        nodes={nodes}
        onNodeClick={(_, node) => {
          const pid = (node.data as Record<string, unknown>)?.panelId as string | undefined;
          if (pid) selectPanel(pid);
        }}
